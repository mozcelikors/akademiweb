...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE hakkinda SET icerik=%s WHERE id=%s",
                       GetSQLValueString($_POST['duyuruaciklamasi'], "text"),
                       GetSQLValueString($_POST['id'], "int"));

  mysql_select_db($database_yunusnet1, $yunusnet1);
  $Result1 = mysql_query($updateSQL, $yunusnet1) or die(mysql_error());
...
$colname_hakkinda = "-1";
if (isset($_GET['id'])) {
  $colname_hakkinda = (get_magic_quotes_gpc()) ? $_GET['id'] : addslashes($_GET['id']);
}
mysql_select_db($database_yunusnet1, $yunusnet1);
$query_hakkinda = sprintf("SELECT * FROM hakkinda WHERE id = %s", $colname_hakkinda);
$hakkinda = mysql_query($query_hakkinda, $yunusnet1) or die(mysql_error());
$row_hakkinda = mysql_fetch_assoc($hakkinda);
$totalRows_hakkinda = mysql_num_rows($hakkinda);
?>
...
<div id="cevreleyen">
  ...
  <div id="icerik">
  <form action="<?php echo $editFormAction; ?>" onsubmit="return kontrol()" id="form1" name="form1" method="POST">
    <table width="99%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="11%" height="52">&nbsp;</td>
        <td width="56%"><h1><strong>Hakkýnda Bölümünü Düzenle </strong></h1></td>
      ...
      <tr>
        ...
        <td bgcolor="#FFFFFF"><div class="style5 style6"><textarea name="duyuruaciklamasi" cols="80" rows="12" class="giris" id="formalani"><?php echo $row_hakkinda['icerik']; ?></textarea></div></td>
        <td>&nbsp;</td>
      </tr>
      <tr>
        <td>&nbsp;</td>
      ...
      <tr>
        <td height="57">&nbsp;</td>
        <td><input name="id" type="hidden" id="id" value="<?php echo $row_hakkinda['id']; ?>" />
          <input name="onay" type="hidden" id="onay" value="evet" />
          <input type="submit" id="formGonder" name="Submit" value="Düzenle" /></td>
        <td>&nbsp;</td>
      </tr>
    </table>
